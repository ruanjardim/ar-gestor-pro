...
import {
  ...
  CreditCard, Download, FileText, FileUp, Gem, KeyRound, LayoutDashboard, LogOut, Menu,
...
type View = 'dashboard'|'clients'|'categories'|'search'|'due'|'notes'|'reports'|'charts'|'reminders'|'export'|'whatsapp'|'messages'|'pix'|'billing'|'team'|'organizations'|'profile';
export type CurrentUser = { id:number; organizationId:number; organizationName:string; name:string; email:string; role:'master'|'member'; platformAdmin:boolean };
type TeamUser = { id:number; name:string; email:string; role:string; status:string; createdAt:string; lastLoginAt:string|null };
...
const views: Array<{key:View; label:string; icon:typeof LayoutDashboard; masterOnly?:boolean; platformOnly?:boolean}> = [
  ...
  {key:'pix',label:'PIX',icon:Gem},{key:'billing',label:'Assinatura',icon:CreditCard},
...
function Navigation({view,onSelect,currentUser}:{view:View;onSelect:(key:View)=>void;currentUser:CurrentUser}) {
  ...
  return <SidebarMenu className="nav-menu">{views.filter(item=>(!item.masterOnly||currentUser.role==='master')&&(!item.platformOnly||currentUser.platformAdmin)).map(({key,label,icon:Icon})=><SidebarMenuItem key={key}><SidebarMenuButton isActive={view===key} tooltip={label} onClick={()=>select(key)}><Icon/><span>{label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu>;
}
...
export default function GestorApp({currentUser,onLogout}:{currentUser:CurrentUser;onLogout:()=>void}) {
  ...
  const viewContent:{[K in View]:React.ReactNode}={dashboard,clients,categories,search,due,notes,reports,charts,reminders,export:exportView,whatsapp,messages,pix,billing,team:teamView,organizations:organizationsView,profile};
